...
@pytest.mark.parametrize("price, ex_rate, expected", [
    (100., 20., 2730.0),
    (1000., 20., 27300.0),
    (50, 19, 1296.75),
])
def test_calc_selling_price(price, ex_rate, expected):
    out = parse.calc_selling_price(price, ex_rate)
    assert out == expected


@pytest.mark.parametrize("message, expected", [
    ({"price": 100., "vat": True, "lead_days": 1, "currency": "AED"},
     {"price": 2730., "lead_days": 15, "currency": "RUB", "part_number": None}),
])
...
@pytest.mark.parametrize("message, expected", [
    ("1000 + vat 1 month order", [{"price": 27300.0, "lead_days": 44, "currency": "RUB", "part_number": None}]),
    ("""A1678853300 - 900 + vat  7-10 days 
 A1678802808 - 540 + vat  17-21 days""",
     [{"price": 24570., "lead_days": 24, "currency": "RUB", "part_number": "A1678853300"},
        {"price": 14742.0, "lead_days": 35, "currency": "RUB", "part_number": "A1678802808"}])
])
