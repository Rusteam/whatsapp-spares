...
@pytest.mark.parametrize("part_number, expected", [
    ("A1679063107", 0.919),
    ("A0259975047", 0.087),
])
def test_get_weight(mocker, part_number, expected):
    mocker.patch("bot.services._get_fixparts_entries")
    mocker.patch("bot.services._extract_fixparts_linkpath",
                 return_value=part_number)
    mocker.patch("bot.services._get_fixparts_product_page",
